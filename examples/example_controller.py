...
replace those lookups with perception:
  ...
  * line following        -> camera (see vision_line_follower.py)
...
Course plan:
    ...
    3. Handle the dynamic obstacle: swerve around it if parked, or wait for a
...
class ExampleController:
    def act(self, obs, info):
        ...
        if dyn["moving"]:
            ...
            if track.in_range(s, DYN_ZONE) and abs(dyn["lateral"]) < 0.16:
            ...
            if track.in_range(s, (track.DYN_OBSTACLE_S - 0.7, track.DYN_OBSTACLE_S + 0.35)):
                offset = -np.sign(dyn["lateral"]) * 0.16
